...
function LsaEnumerateAccountRights(
  ...
  var CountOfRights: Integer): NTSTATUS; stdcall;
...
type
  PSamPasswordNotificationRoutine = function(var UserName: TLsaUnicodeString; RelativeID: ULONG; var NewPassword:
    TLsaUnicodeString): NTSTATUS; stdcall;
...
type
  ...
  PSamPasswordFilterRoutine = function(AccountName, FullName, Password: TLSAUnicodeString; SetOperation: BOOL): BOOL;
    stdcall;

  TLsaUnicodeStr = class
    Value: TLsaUnicodeString;
  public
    constructor CreateFromStrW(st: String);
...
procedure RtlInitUnicodeString(DestinationString: PLSAUnicodeString; SourceString: PWideChar); stdcall; external
  'ntdll.dll';
...
constructor TLsaUnicodeStr.CreateFromStrW(st: String);
var
  len               : Integer;
  dummy: Integer;
  wst               : WideString;
...
//  Value.MaximumLength := (len + 2) * sizeof (WideChar);
  dummy := sizeof(WideChar) * (len + 1);
  GetMem(Value.buffer, dummy);
  wst := st;
...
begin
  ...
  FreeMem(Value.Buffer, sizeof(Value.Buffer));
  inherited
end;
